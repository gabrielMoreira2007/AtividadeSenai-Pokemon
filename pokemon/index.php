...
<body>
  <div class="overlay">
    ...
    if ($resumo->num_rows > 0) {
      ...
      echo "<p style='text-align:center;'>Nenhum Pokémon cadastrado ainda.</p>";
    }
    ?>
    ...
    <form method="POST" enctype="multipart/form-data">
      <label>Nome*: <input type="text" name="nome" required value="<?php echo isset($editRow) ? htmlspecialchars($editRow['nome']) : ''; ?>"></label>
      <label>Tipo: <input type="text" name="tipo" value="<?php echo isset($editRow) ? htmlspecialchars($editRow['tipo']) : ''; ?>"></label>
      <label>Localização Encontrada: <input type="text" name="local" value="<?php echo isset($editRow) ? htmlspecialchars($editRow['localizacao']) : ''; ?>"></label>
      <label>Data do Registro: <input type="date" name="data" value="<?php echo isset($editRow) ? $editRow['data_registro'] : ''; ?>"></label>
      <label>HP: <input type="number" name="hp" value="<?php echo isset($editRow) ? $editRow['hp'] : ''; ?>"></label>
      <label>Ataque: <input type="number" name="ataque" value="<?php echo isset($editRow) ? $editRow['ataque'] : ''; ?>"></label>
      <label>Defesa: <input type="number" name="defesa" value="<?php echo isset($editRow) ? $editRow['defesa'] : ''; ?>"></label>
      <label>Observações: <textarea name="obs"><?php echo isset($editRow) ? htmlspecialchars($editRow['observacoes']) : ''; ?></textarea></label>
      <label>Foto (upload): <input type="file" name="foto"></label>
      <label>Ou link da foto: <input type="text" name="foto_link" value="<?php echo isset($editRow) ? htmlspecialchars($editRow['foto']) : ''; ?>"></label>
    ...
    if (isset($_GET['excluir'])) {
      ...
      echo "<p style='text-align:center; color: green;'>Pokémon excluído com sucesso!</p>";
    }
    ...
    if (isset($_POST['atualizar'])) {
      ...
      $stmt->execute();
      echo "<p style='text-align:center; color: green;'>Pokémon atualizado com sucesso!</p>";
    }
    ...
    if (isset($_POST['cadastrar'])) {
      ...
      if (!empty($nome)) {
        ...
        $stmt->execute();
        echo "<p style='text-align:center; color: green;'>Pokémon cadastrado com sucesso!</p>";
      } else {
        echo "<p style='text-align:center; color: red;'>O nome é obrigatório!</p>";
      }
    }
    ?>
    ...
    if ($resultado->num_rows > 0) {
      ...
      while ($row = $resultado->fetch_assoc()) {
        ...
        if (!empty($row['foto'])) {
          echo "<img src='" . htmlspecialchars($row['foto']) . "' alt='Foto' />";
        }
        ...
        echo "<a href='?editar=" . $row['id'] . "'>Editar</a> | ";
        echo "<a href='?excluir=" . $row['id'] . "' onclick=\"return confirm('Tem certeza que deseja excluir?');\">Excluir</a>";
        echo "</div>";
      }
      echo "</div>";
    } else {
      echo "<p style='text-align:center;'>Nenhum pokémon encontrado.</p>";
    }
    ?>
  </div>
